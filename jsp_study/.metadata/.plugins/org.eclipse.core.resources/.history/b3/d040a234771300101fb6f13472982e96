...
<%-- 
1. 로그인 상태 검증
   로그아웃 상태 : 로그인하세요 메세지, loginForm.jsp 페이지로 이동
   로그인 상태 : 
   	- 다른 id를 조회할수 없음. 단 관리자는 다른 id 조회가능함
   	  내 정보만 조회 가능합니다. 메세지 출력. main.jsp 페이지 이동
...
<% } else if(!id.equals(login) && !login.equals("admin")) {  // 로그인이 된 상태 %> 
<script>
	// id != admin
...
<% } else { // 로그인이 된 경우. 자기정보 또는 관리자인 경우  %>
<%
